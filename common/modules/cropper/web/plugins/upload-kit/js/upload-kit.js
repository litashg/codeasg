    ...
    jQuery.fn.yiiUploadKit = function(options) {
        ...
        var methods = {
            ...
            fileuploadInit: function(){
                var $fileupload = $input.fileupload({
                    ...
                    always: function (e, data) {
                        ...
                        $container
                            .find('.upload-kit-input').find('input.crop').remove();
                    }
                ...
                if(options.crop && options.crop === true) {
                    $fileupload
                        .bind('fileuploadadd', function (e, data) {
                            var modal = $container.parent('div').find('#crop-modal');
                            var reader = new FileReader();
                            reader.readAsDataURL(data.files[0]);
                            reader.onload = function() {
                                modal.find('img#crop-cover-photo').attr('src', this.result);
                                modal.modal('show');
                            };

                            var aspectRatio = (options.cropper && options.cropper.aspectRatio) ? options.cropper.aspectRatio : 16/9;

                            var $image = modal.find('#crop-cover-photo');
                            ...
                            modal.find('#save').click(function() {
                                ...
                                data.formData = {
                                    'cropData[x]': Math.abs(Math.ceil(selection.x)),
                                    'cropData[y]': Math.abs(Math.ceil(selection.y)),
                                    'cropData[width]': selection.width,
                                    'cropData[height]': selection.height
                                };
            ...
            showError: function(error){
                ...
                $container
                    .append('<div class="ui error message" style="display: inline-block; min-width: 100%">\n' +
                        '<p>' + error + '</p>' +
                    '</div>');
            },
